...
addUpdateTicker(() => {
  ...
  screen.drawText(screen.width / 2, 8, "Input", Color.Amber300, {
    align: "center",
    scale: 2,
  });
  ...
  if (trail.length > 1) screen.drawPolyline(trail, Color.Slate700, 1);
  screen.drawLine(0, pointer.y + 0.5, screen.width, pointer.y + 0.5, Color.Slate800, 1);
  screen.drawLine(pointer.x + 0.5, 0, pointer.x + 0.5, screen.height, Color.Slate800, 1);
  screen.strokeCircle(pointer.x, pointer.y, isPointerDown() ? 18 : 10, Color.Amber400, 2);
  screen.fillCircle(pointer.x, pointer.y, 3, Color.Amber300);

  screen.drawText(40, 46, "pointer", Color.Slate400);
  screen.drawText(40, 62, `position  ${Math.round(pointer.x)}, ${Math.round(pointer.y)}`, Color.Slate200);
  screen.drawText(40, 78, `moved     ${Math.round(delta.x)}, ${Math.round(delta.y)}`, Color.Slate200);
  screen.drawText(
    40,
    94,
  ...
  screen.drawText(40, 110, `pressed   ${clicks} times`, Color.Slate200);
  screen.drawText(40, 126, `released  ${releases} times`, Color.Slate200);

  // The wheel reports how far it turned this frame, so a program that wants
  // a running total keeps one itself, the way this does.
  screen.drawText(300, 46, "wheel", Color.Slate400);
  screen.drawText(300, 62, `total  ${scrolled.toFixed(1)}`, Color.Slate200);
  const barY = 82;
  screen.fillRectangle(300, barY, 200, 10, Color.Slate800);
  const knob = Math.max(0, Math.min(190, 95 + scrolled * 8));
  screen.fillRectangle(300 + knob, barY, 10, 10, Color.Rose400);

  screen.drawText(300, 110, "this frame", Color.Slate500);
  screen.drawText(300, 126, getScrollDelta().toFixed(2), Color.Slate200);

  screen.drawText(40, 160, "keys — filled while held, counted on each press", Color.Slate400);
  for (const watched of WATCHED) {
    const w = watched.wide ? 92 : 40;
    const x = 40 + watched.x * 48;
    const y = 182 + watched.y * 48;
    const down = isKeyDown(watched.key);
    screen.fillRoundedRectangle(x, y, w, 40, 6, down ? Color.Teal500 : Color.Slate800);
    screen.drawText(x + w / 2, y + 8, watched.label, down ? Color.Slate900 : Color.Slate300, {
      align: "center",
    });
    screen.drawText(
      x + w / 2,
