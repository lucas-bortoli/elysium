...
function windmill(color: Color) {
  for (let i = 0; i < 4; i++) {
    screen.pushTransform({ rotate: (i * Math.PI) / 2 });
    screen.fillRoundedRectangle(6, -4, 26, 8, 3, color);
    screen.popTransform();
  }
  screen.fillRectangle(-3, -3, 6, 6, Color.Slate200);
}

addUpdateTicker(() => {
  screen.clear(Color.Slate900);
  screen.drawText(screen.width / 2, 8, "Transforms", Color.Amber300, {
    align: "center",
    scale: 2,
  });

  screen.drawText(40, 42, "the same windmill, drawn at the origin every time", Color.Slate400);

  // Shifted only.
  screen.pushTransform({ translate: { x: 90, y: 110 } });
  windmill(Color.Teal400);
  screen.popTransform();
  screen.drawText(90, 155, "translate", Color.Slate500, { align: "center" });

  // Shifted and turned.
  screen.pushTransform({ translate: { x: 220, y: 110 }, rotate: elapsed });
  windmill(Color.Teal400);
  screen.popTransform();
  screen.drawText(220, 155, "+ rotate", Color.Slate500, { align: "center" });
  ...
  windmill(Color.Teal400);
  screen.popTransform();
  screen.drawText(360, 155, "+ scale", Color.Slate500, { align: "center" });
  ...
  windmill(Color.Teal400);
  screen.popTransform();
  screen.drawText(500, 155, "uneven scale", Color.Slate500, { align: "center" });

  screen.drawText(40, 190, "a camera on the outside, objects placing themselves inside it", Color.Slate400);
  ...
  screen.strokeRectangle(-300, -60, 600, 120, Color.Slate700, 2);
  for (let i = 0; i < 7; i++) {
    const x = -250 + i * 83;
    // Each windmill nests its own transform inside the camera's, so it
    // spins about itself while the camera moves the lot.
    screen.pushTransform({ translate: { x, y: 0 }, rotate: elapsed * (i % 2 ? -1 : 1) });
