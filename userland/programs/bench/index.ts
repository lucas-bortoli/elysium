...
function rects(): void {
  ...
  for (let i = 0; i < 3000; i++) {
    fillRectangle(rnd() * WIDTH, rnd() * HEIGHT, 4 + rnd() * 40, 4 + rnd() * 40, pick(PALETTE));
  }
}
...
function circles(): void {
  ...
  for (let i = 0; i < 1000; i++) {
    fillCircle(rnd() * WIDTH, rnd() * HEIGHT, 3 + rnd() * 20, pick(PALETTE));
  }
  for (let i = 0; i < 1000; i++) {
    strokeCircle(rnd() * WIDTH, rnd() * HEIGHT, 3 + rnd() * 20, pick(PALETTE), 2);
  }
}
...
function rounded(): void {
  ...
  reseed();
  for (let i = 0; i < 800; i++) {
    fillRoundedRectangle(rnd() * WIDTH, rnd() * HEIGHT, 20 + rnd() * 60, 16 + rnd() * 40, 6, pick(PALETTE));
  }
  for (let i = 0; i < 800; i++) {
    strokeRoundedRectangle(rnd() * WIDTH, rnd() * HEIGHT, 20 + rnd() * 60, 16 + rnd() * 40, 6, pick(PALETTE), 2);
  }
}
...
function lines(): void {
  ...
  reseed();
  for (let i = 0; i < 3000; i++) {
    drawLine(rnd() * WIDTH, rnd() * HEIGHT, rnd() * WIDTH, rnd() * HEIGHT, pick(PALETTE), 1 + rnd() * 3);
  }
}
...
function arcs(): void {
  ...
  for (let i = 0; i < 1200; i++) {
    ...
    drawArc(rnd() * WIDTH, rnd() * HEIGHT, 6 + rnd() * 24, a, a + 1 + rnd() * 4, pick(PALETTE), 2);
  }
}
...
function text(): void {
  ...
  reseed();
  for (let i = 0; i < 320; i++) {
    drawText(rnd() * WIDTH, rnd() * HEIGHT, pick(WORDS), pick(PALETTE));
  }
  drawText(20, 20, WORDS.join(" ") + "\n" + WORDS.join(" "), Color.Slate500, {
    maxWidth: 300,
  });
}
...
function spritesScaled(): void {
  ...
  reseed();
  for (let i = 0; i < 400; i++) {
    drawImage(photo, rnd() * WIDTH - 32, rnd() * HEIGHT - 32, {
      sx: rnd() * (540 - 64),
      sy: rnd() * (360 - 64),
      sw: 64,
      sh: 64,
...
function spritesRotated(): void {
  ...
  for (let i = 0; i < 400; i++) {
    drawImageRotated(photo, rnd() * WIDTH, rnd() * HEIGHT, angle + rnd() * 6, {
      sx: rnd() * (540 - 48),
      sy: rnd() * (360 - 48),
      sw: 48,
      sh: 48,
...
function transforms(): void {
  ...
  for (let i = 0; i < 900; i++) {
    ...
    fillRectangle(-10, -10, 20, 20, pick(PALETTE));
    popTransform();
  }
}
...
function clipsRect(): void {
  ...
  for (let i = 0; i < 300; i++) {
    ...
    pushClip(x, y, w, h);
    for (let j = 0; j < 6; j++) {
      fillCircle(x + 20 + rnd() * (w - 40), y + 20 + rnd() * (h - 40), 6 + rnd() * 14, pick(PALETTE));
    }
    popClip();
  }
}
...
function clipsOverflow(): void {
  ...
  for (let i = 0; i < 300; i++) {
    const x = rnd() * WIDTH;
    const y = rnd() * HEIGHT;
    pushClip(x, y, 40 + rnd() * 80, 40 + rnd() * 80);
    for (let j = 0; j < 6; j++) {
      fillCircle(x + rnd() * 100, y + rnd() * 100, 20 + rnd() * 30, pick(PALETTE));
    }
    popClip();
  }
}
...
function clipsRotated(): void {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    pushClip(-40, -40, 80, 80);
    for (let j = 0; j < 8; j++) {
      fillRectangle(-60 + rnd() * 120, -60 + rnd() * 120, 30, 30, pick(PALETTE));
    }
    popClip();
    popTransform();
  }
...
function paths(): void {
  ...
  for (let i = 0; i < 220; i++) {
    ...
    fillPath(pick(PALETTE), "evenodd");
    strokePath(Color.Slate950, 1.5);
  }
}
...
function mixed(): void {
  ...
  for (let i = 0; i < 70; i++) {
    drawImage(photo, rnd() * WIDTH - 24, rnd() * HEIGHT - 24, {
      sx: rnd() * (540 - 48),
      sy: rnd() * (360 - 48),
      sw: 48,
      sh: 48,
  ...
  for (let i = 0; i < 24; i++) {
    drawImageRotated(photo, rnd() * WIDTH, rnd() * HEIGHT, frame * 0.05 + rnd() * 6, {
      sx: 0,
      sy: 0,
      sw: 40,
      sh: 40,
  ...
  pushClip(12, 12, 240, 90);
  fillRoundedRectangle(12, 12, 240, 90, 8, Color.Slate800);
  for (let i = 0; i < 20; i++) {
    fillCircle(24 + i * 11, 60 + Math.sin(frame * 0.1 + i) * 20, 5, pick(PALETTE));
  }
  popClip();
  ...
  for (let i = 0; i < 40; i++) {
    drawLine(rnd() * WIDTH, rnd() * HEIGHT, rnd() * WIDTH, rnd() * HEIGHT, pick(PALETTE), 1);
  }
  for (let i = 0; i < 12; i++) {
    star(rnd() * WIDTH, rnd() * HEIGHT, 10 + rnd() * 18);
    fillPath(pick(PALETTE));
  }
}
